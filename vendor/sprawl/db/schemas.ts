...
export interface IActiveGig {
  ...
  crewIds?: string[];
  /** Scripted nodes may move the crew without an exit. */
  autoPlot?: boolean;
}
...
/** Map REA / shooting / reaction → reaction. Null for resilience, etc. */
export function canonStat(raw: unknown): StatKey | null {
  const key = String(raw ?? "").trim().toLowerCase();
  if (!key) return null;
  return STAT_ALIASES[key] ?? null;
}

function statNum(value: unknown): number | null {
...
/** Accept full names, MOR/REA, or mixed-case keys from old sheets. */
export function readStats(raw: unknown): ISprawlStats {
  const next = emptyStats();
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return next;
  const rec = raw as Record<string, unknown>;
  for (const key of STAT_KEYS) {
    const n = statNum(rec[key]);
    if (n != null) next[key] = n;
  }
  for (const [key, value] of Object.entries(rec)) {
    const stat = canonStat(key);
    const n = statNum(value);
    if (!stat || n == null) continue;
    if (statNum(rec[stat]) != null) continue;
    next[stat] = n;
  }
  return next;
}
...
export function readSprawl(
  ...
  const c = raw as ISprawlChar;
  return { ...c, stats: readStats(c.stats) };
}

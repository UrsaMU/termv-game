...
import { mailChargenApproved, sendSprawlMail } from "../integrations/mail.ts";

function bare(id: string): string {
  return String(id ?? "").replace(/^#/, "").trim();
}

export { parseWhoNote } from "./approve_parse.ts";

export async function playerIdFromCgenJob(job: {
  submittedBy?: string;
  submitterName?: string;
}): Promise<string | null> {
  const by = bare(String(job.submittedBy ?? ""));
  if (by) {
    try {
      const row = await dbojs.queryOne({ id: by });
      if (row) return by;
    } catch {
      /* try name */
    }
  }
  const name = String(job.submitterName ?? "").trim();
  if (!name) return by || null;
  try {
    ...
    for (const row of all) {
      ...
      const n = String(rec.name ?? rec.data?.name ?? "").trim();
      if (n && n.toLowerCase() === want) {
        return bare(String(rec.id ?? ""));
      }
    }
  } catch (e: unknown) {
    console.error("[sprawl] playerIdFromCgenJob:", e);
  }
  return by || null;
}
...
> {
  ...
  if (c.chargenComplete || c.chargenStatus === "approved") {
    ...
    return { ok: true, name, already: true, job: jobNum };
  }

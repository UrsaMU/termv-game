...
const onReady = () => {
  ...
  // Starter Flow: OPEN districts + haunts (idempotent)
  void import("./engine/city-seed.ts")
    .then((m) => m.seedStarterCity())
    .then((r) => {
      if (r.errors.length) {
        console.warn("[sprawl] city seed:", r.errors.join("; "));
      } else if (r.created.length || r.linked.length) {
        console.log(
          "[sprawl] city seed OPEN:",
          r.linked.join(", ") || "already linked",
        );
      }
    })
    .catch((e) => {
      console.warn("[sprawl] city seed failed:", e);
    });
};

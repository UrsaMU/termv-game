...
import { emitSprawl, prefersSprawlJson } from "./frame.ts";
...
async function showLook(u: IUrsamuSDK, title: string): Promise<void> {
  ...
  const prose = (await resolveLook(u, u.me, c, items)) || "";
  const { plain } = await import("./chrome.ts");
  const countName = plain(String(c.name || "GOON")).trim() ||
    "GOON";
  const framed = frameStreetLook(
    prose || dim("No look yet. Try +desc/roll."),
    { name: countName },
  );
...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (sw === "set") {
      if (!arg || arg.length > 2000) {
        u.send(`${ERR}Need text (max 2000).`);
        return;
      }
      const { items } = await getInventory(u, u.me);

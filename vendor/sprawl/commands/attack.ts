...
import { onGigHostileDown } from "../engine/gig-run.ts";
import {
  combatFlavorLine,
  flavorEnabled,
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (
      ...
      if (pre.hit?.dead) {
        ...
        const down = await onGigHostileDown(u, sheet, npcObj);
        sheet = down.next;
        if (down.note?.includes("TARGET")) gigTokenNote = down.note;
        else if (down.note) gigNodeNote = down.note;
      }
    }
    if (
    ...
    if (
      ...
      if (sheet.activeGig && (gigNodeNote || gigTokenNote)) {
        const { emitGigFrame } = await import("./client-sync.ts");
        emitGigFrame(u, sheet.activeGig, gigNodeNote || gigTokenNote);
      }
    }
    ...
    if (loot && (loot.bityuan > 0 || loot.ap > 0)) {
      const { emitPayout } = await import("./client-sync.ts");
      emitPayout(u, {
        kind: "kill",
        label: loot.label,
        bityuan: loot.bityuan,
        ap: loot.ap,
      }, formatLootLine(loot));
    }
    // deno-lint-ignore no-explicit-any

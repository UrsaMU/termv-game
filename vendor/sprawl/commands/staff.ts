...
function hubLines(): string {
  return [
    ...
    `  ${val("+staff/ap")} <player>=<n>` +
      `     ${dim("grant AP + level")}`,
    `  ${val("+staff/seed-city")}` +
      `        ${dim("OPEN starter districts + haunts")}`,
    `  ${dim("Also:")}`,
...
Switches:
  ...
  /seed-city      OPEN starter districts + haunts
...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (
      sw === "seed-city" || sw === "seedcity" || sw === "city-seed" ||
      sw === "seed"
    ) {
      const {
        formatSeedReport,
        seedStarterCity,
      } = await import("../engine/city-seed.ts");
      const report = await seedStarterCity();
      u.send(
        [
          header("CITY SEED"),
          ...formatSeedReport(report).map((l) => `  ${l}`),
          `  ${dim("+flow · +haunt — OPEN sectors live")}`,
          footer("STAFF"),
        ].join("\r\n"),
      );
      return;

...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    const text = await renderLoadoutView(u);
    await emitGearFrame(u, text);
  },
});
...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    // Pack ops (list/wear/wield) must work while the sheet is still
    // submitted — requireChar waits on staff approval.
    const c = getChar(u.me);
    if (!c) {
      u.send(

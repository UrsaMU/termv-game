...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (!sw) {
      ...
      {
        const { emitNetFrame } = await import("./client-sync.ts");
        emitNetFrame(u, c, lines.join("\r\n"));
      }
      return;
    }
    if (sw === "status" || sw === "net") {
...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    let lockRef = "";
    {
      const nameTok = pooled.rest.find((t) =>
        t &&
        !t.startsWith("+") &&
        !/^\d+$/.test(t) &&
        t !== "glitch" &&
      ...
      if (nameTok) {
        const locks = await loadRoomHackLocks(u);
        const hit = matchHackLock(locks, nameTok);
        const hl = hit ? hackLockOf(hit) : null;
        if (hl) {
          ds = hl.ds;
          lockRef = nameTok;
        }
    ...
    if (result.success) {
      ...
      const doorHack = await tryHackLockAfterRoll(
        u,
        lockRef || ref,
        true,
      );
      for (const n of doorHack.notes) {
        lines.push(`  ${good(n)}`);
      }
    }

  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (sw === "status" || sw === "show" || sw === "card") {
      ...
      }
      {
        ...
        const { emitGigFrame } = await import("./client-sync.ts");
        emitGigFrame(u, g, text);
      }
      return;
    }
      ...
      try {
        ...
        await syncGigToCrew(u, next);
        {
          const text = [
            ...
            ...popMsgs.map((m) => `  ${m}`),
            `  ${dim(
              "look · +attack · +hack · +gig/leave",
            )}`,
          ].join("\r\n");
          const liveGig = next.activeGig;
          if (liveGig) {
            const { emitGigFrame } = await import("./client-sync.ts");
            emitGigFrame(u, liveGig, text, { onSite: true });
          } else {
            u.send(text);
          }
        }
    ...
    if (sw === "leave" || sw === "exit" || sw === "out") {
      ...
      await saveChar(u, next);
      {
        ...
        if (g) {
          const { emitGigFrame } = await import("./client-sync.ts");
          emitGigFrame(u, g, text, { status: "left", onSite: false });
        } else {
          u.send(text);
        }
      }
    ...
    if (sw === "abandon" || sw === "cancel") {
      ...
      {
        const { emitGigAbandoned } = await import("./client-sync.ts");
        emitGigAbandoned(u, `${OK}Gig abandoned. Site wiped. No pay.`);
      }
      return;
    }
    ...
    if (sw === "push" || sw === "next" || sw === "advance") {
      ...
      await syncGigToCrew(u, r.next);
      {
        ...
        const nextGig = r.next.activeGig;
        if (nextGig) {
          const { emitGigFrame } = await import("./client-sync.ts");
          emitGigFrame(u, nextGig, text);
        } else {
          u.send(text);
        }
      }
    ...
    if (sw === "turnin" || sw === "complete" || sw === "done") {
      ...
      const out = [
        ...
        `  ${dim(
          next.ap >= cost
            ? "+advance/<track> when you want — not required"
            : "bank AP toward +advance",
        )}`,
        `  ${ylw("NEXT —")}` +
          ` ${val("MARKET")} · ${val("HAUNTS")} · ${val("MAP")} · ` +
          `${val("+gig")} again`,
        `  ${dim(
          "Spend chrome, slide a booth, jack a district, or pull another run.",
        )}`,
      ];
      if (obs.died.length) {
        out.push(
      ...
      }
      {
        ...
        emitGigFrame(
          ...
          out.join("\r\n"),
          {
            ...
            token: false,
            onSite: false,
            payoutBy: reward.bityuan,
            payoutAp: reward.ap,
            nextHint:
              "NEXT — MARKET · HAUNTS · MAP · +gig again",
          },
        );
        const { emitPayout } = await import("./client-sync.ts");
        emitPayout(
          u,
          {
            kind: "gig",
            label: gig.title,
            bityuan: reward.bityuan,
            ap: reward.ap,
          },
          `+${reward.bityuan} b¥ · +${reward.ap} AP`,
        );
      ...
      if (
            ...
            u.here?.id === g.siteRoomId);
        {
          const text = [
            ...
            onSite
              ? `  ${ylw("You are on-site.")} ` +
                `${dim("look · +attack · +hack — take the exit when the room is clear")}`
              : `  ${ylw(">>> +gig/enter <<<")} ` +
    ...
    const max = gig.nodesMax ?? 1;
    {
      const text = [
        ...
        ` Room clear moves you. Last room: +gig/turnin.`,
        footer(),
      ].join("\r\n");
      const { emitGigFrame } = await import("./client-sync.ts");
      emitGigFrame(u, gig, text);
    }
  },
});

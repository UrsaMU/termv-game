...
import { emitGearFrame } from "./client-sync.ts";
import { prefersSprawlJson } from "./frame.ts";
...
function suggestFor(kind: string, hint: { magMax?: number; slug?: string; name?: string } = {}): "wear" | "wield" | "use" {
  const k = kind.toLowerCase().trim();
  const blob = [k, hint.slug, hint.name].filter(Boolean).join(" ").toLowerCase();
  if (
    k === "armor" || k === "armour" || k === "drone" || k === "clothes" ||
    /\b(armou?r|vest|leathers?|jacket|helmet|duster)\b/.test(blob)
  ) {
    return "wear";
  }
  if (
    k === "firearm" || k === "firearms" || k === "melee" || k === "heavy" ||
    k === "weapon" || k === "gun" || k === "guns" ||
    hint.magMax != null ||
    /\b(firearm|pistol|rifle|gun|melee|blade|knife|sword|katana|weapon|holdout)\b/.test(blob)
  ) {
    return "wield";
  }
  return "use";
...
export async function setSlotSwitch(
  ...
  if (sw === "wear" && suggestFor(d0.kind, { magMax: d0.magMax, slug: d0.slug, name: displayName(item) }) === "wield") {
    u.send(
  ...
  if (sw === "wield" && suggestFor(d0.kind, { magMax: d0.magMax, slug: d0.slug, name: displayName(item) }) === "wear") {
    u.send(

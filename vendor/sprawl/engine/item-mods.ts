...
function catalogForMod(
  ...
  return matchModRow(slug) ?? find("mod", slug) ?? find("vehicleMod", slug);
}
...
export async function attachMod(
  ...
  const hd = itemDataRepaired(host) ?? itemData(host);
  if (!hd) return { ok: false, error: "Not a Sprawl item." };
  const hk = presentedItemKind(hd, displayName(host)).toLowerCase();
  ...
  const md = itemDataRepaired(loose) ?? itemData(loose);
  const mk = md ? presentedItemKind(md, displayName(loose)) : "";

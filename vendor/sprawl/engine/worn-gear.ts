...
import { canonStat } from "../db/schemas.ts";
...
export function wornStatBonuses(
  ...
  for (const raw of items) {
    ...
    for (const sm of d.statMods) {
      if (!want || canonStat(sm.stat) !== want) continue;
...
export function parseStatMods(
  ...
  if (Array.isArray(raw)) {
    for (const row of raw) {
      ...
      const stat = canonStat(r.stat) ?? String(r.stat ?? "").toLowerCase();
  ...
  if (!out.length && legacyStat != null && legacyMod != null) {
    ...
    if (mod) {
      out.push({
        stat: canonStat(legacyStat) ?? String(legacyStat).toLowerCase(),
        mod,
      });
    }
  }

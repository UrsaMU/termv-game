...
const MOD_ALIASES: Record<string, string> = {
  "smart-link": "smart-targeting",
  "smartlink": "smart-targeting",
  "smart-link-system": "smart-targeting",
};

export function matchModRow(
  slug: string,
  name?: string,
): Row | undefined {
  const raw = slug.toLowerCase().trim();
  if (!raw) return undefined;
  const alias = MOD_ALIASES[raw] ?? raw;
  const hit = find("mod", alias) ?? find("mod", raw);
  if (hit) return hit;
  const blob = `${raw} ${name ?? ""}`.toLowerCase();
  if (/smart-?link|smart.?target/.test(blob)) {
    return find("mod", "smart-targeting");
  }
  const want = (name ?? "").toLowerCase().trim();
  return WEAPON_MODS.find((r) =>
    r.slug === alias ||
    (want && String(r.name ?? "").toLowerCase() === want)
  );
}
...
export function repairItemData(
  ...
  const blankKind = k0 === "gear" || k0 === "weapon" ||
    !String(next.kind ?? "").trim();
  const modRow = blankKind
    ? matchModRow(next.slug, opts.name)
    : undefined;
  if (modRow && next.kind !== "mod") {
    next = {
      ...next,
      kind: "mod",
      slug: String(modRow.slug ?? next.slug),
    };
    changed = true;
  }
  const match = matchCombatRow(next.slug, opts.name);
  if (match && next.kind !== "mod") {
    if (
...
/** Kind the client should see — fill blanks so guns/armor aren't "use". */
export function presentedItemKind(
  d: SprawlItemData,
  name = "",
): string {
  const fixed = repairItemData(d, { name }).data;
  const k = String(fixed.kind || "").trim();
  if (k === "mod" || k === "ammo" || k === "vehicle-mod") return k;
  if (matchModRow(fixed.slug, name)) return "mod";
  if (k && k !== "gear") return k;
  const blob = `${fixed.slug} ${name} ${k}`.toLowerCase();
  if (
    fixed.magMax != null ||
    /(firearm|pistol|rifle|shotgun|smg|gun|melee|blade|knife|sword|katana|weapon|holdout)/
      .test(blob)
  ) {
    return /melee|blade|knife|sword|katana/.test(blob) ? "melee" : "firearm";
  }
  if (/(armou?r|vest|leathers?|jacket|helmet|duster)/.test(blob)) {
    return "armor";
  }
  return k || "gear";
}
...
export async function resolveItemRef(
  ...
  const exactSlug = pool.find(
    (o) => (itemData(o)?.slug ?? "").toLowerCase() === lc,
  ) ?? all.find((o) => (itemData(o)?.slug ?? "").toLowerCase() === lc);
